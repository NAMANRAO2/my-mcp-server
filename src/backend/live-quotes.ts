...
let FINNHUB_KEY: string | undefined;
...
const simulatorsRunning = new Set<string>();

function startSimulatedTicker(symbol: string, startPrice: number, startDayChangePct: number, swingPct: number, note?: string) {
  if (simulatorsRunning.has(symbol)) return; // never double-start (e.g. a late Finnhub error after seeding)
...
const NO_LIVE_FEED_NOTE =
  'Simulated. Real-time Indian F&O data requires a paid, broker-linked feed (e.g. Kite Connect, Upstox) — not available on any free API.';

// ---------------------------------------------------------------------------
// Bootstrap
// ---------------------------------------------------------------------------

export async function initLiveQuotes(mockQuotes: Record<string, { price: number; day_change_pct: number }>) {
  ...
  const startEquityFallback = (symbol: string) => {
