...
function valuePortfolio() {
  ...
  const priced = portfolio.holdings.map((h: any) => {
  ...
  const holdings = priced.map(({ h, price, day, mv, cb }: any) => ({
    ...h,
  ...
  const largest = holdings.reduce((a: any, b: any) => (b.weight_of_total > a.weight_of_total ? b : a));
