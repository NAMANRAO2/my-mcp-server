...
export function initPortfolioState(baseline) {
...
function requireState() {
    if (!state)
        throw new Error('Portfolio state not initialised — initPortfolioState() must run at server startup');
    return state;
}
...
function recordTrade(input, cashAfter) {
    tradeCounter += 1;
    const trade = {
